...
void ClientManager::dump_statistics() {
  if (is_memprof_on()) {
    ...
    for (auto &info : v) {
      if (count++ < 50) {
        LOG(INFO) << td::format::as_size(info.size) << td::format::as_array(info.backtrace);
      } else {
        other_size += info.size;
      }
      total_size += info.size;
    }
    LOG(INFO) << td::tag("other", td::format::as_size(other_size));
    LOG(INFO) << td::tag("total size", td::format::as_size(total_size));
    LOG(INFO) << td::tag("total traces", get_ht_size());
    LOG(INFO) << td::tag("fast_backtrace_success_rate", get_fast_backtrace_success_rate());
  }
  auto r_mem_stat = td::mem_stat();
  if (r_mem_stat.is_ok()) {
    auto mem_stat = r_mem_stat.move_as_ok();
    LOG(INFO) << td::tag("rss", td::format::as_size(mem_stat.resident_size_));
    LOG(INFO) << td::tag("vm", td::format::as_size(mem_stat.virtual_size_));
    LOG(INFO) << td::tag("rss_peak", td::format::as_size(mem_stat.resident_size_peak_));
    LOG(INFO) << td::tag("vm_peak", td::format::as_size(mem_stat.virtual_size_peak_));
  }
  LOG(INFO) << td::tag("buffer_mem", td::format::as_size(td::BufferAllocator::get_buffer_mem()));
  LOG(INFO) << td::tag("buffer_slice_size", td::format::as_size(td::BufferAllocator::get_buffer_slice_size()));
  ...
  LOG(INFO) << td::tag("pending queries", query_count) << td::tag("pending requests", query_list_size);
  ...
  for (auto end = &shared_data->query_list_, cur = end->prev; cur != end; cur = cur->prev, i++) {
    if (i < 20 || i > query_list_size - 20 || i % (query_list_size / 50 + 1) == 0) {
      ...
      LOG(INFO) << static_cast<const Query &>(*cur);
    } else {
      was_gap = true;
    }
  }
